# XMC&trade; MCU: GPIO toggle

This code example demonstrates GPIO toggling by periodically blinking an LED using SysTick available in XMC&trade; MCU devices.
...
The following example will clone the "[GPIO Toggle](https://github.com/Infineon/mtb-example-xmc-gpio-toggle)" application with the desired name "GPIOToggle" configured for the *KIT_XMC47_RELAX_V1* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_XMC47_RELAX_V1 --app-id mtb-example-xmc-gpio-toggle --user-app-name GPIOToggle --target-dir "C:/mtb_projects"
   ```
...
Afterwards, the pin is toggled with a frequency of 1 Hz in the `While(1)` loop. The toggle period can be changed by ising `USER_LED_TOGGLE_PERIOD_MS` defined as a global variable.
